var summernote_imgUpload = {

	uploadImg(eleObj,contentObjId) {
		...
		imgUpload.ajax_upload(elementId, function (dataResult) {
			...
			if (dataResult.status == 1000) {
				...
				imgUpload.cutPic({
					...
					"w": w,
                    "eleId":elementId,
                    "contentEleId":contentObjId
				}, function (dataImg) {
					var imgUrl=dataImg.data;
                    var imgNode = document.createElement('img');
                    $(imgNode).attr("src",imgUrl);
                    $(contentObj).summernote('insertNode', imgNode);
				});
			}
	...
	cutPic(cutArea, callback) {
		var parentId=$("#"+cutArea.contentEleId).attr("data-cutImg-ParentId");
