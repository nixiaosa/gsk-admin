var page = new Vue({
    ...
    methods: {
        ...
        save_pro_info: function (proStatus) {
            ...
            var _this = this;
            HttpUtils.requestPost("/api/live/dk_save", JSON.stringify(_this.pro_info), function (dataResult) {

                if (dataResult.status == 1000) {
                    //_this.find_pro_info(dataResult.data);
                    $.toast("保存成功!");
                    router.push({path:'live_dk_list',query:{uuid:''}});
                } else {
                    HttpUtils.showMessage("请填写完整的讲堂信息");
                }
            });
        },
