var page = new Vue({
    ...
    methods:{
        search_pro:function(isSearch){
            ...
            HttpUtils.requestPost("/api/material/list",JSON.stringify(jsonData),function(dataResult){
        ...
        save_pro_status:function(index,event){
            ...
            HttpUtils.requestPost("/api/material/saveStatus",JSON.stringify(jsonData),function(dataResult){
                if(dataResult.status==1000){
        ...
        link_pro_detail:function(index){
            var uuid=this.product_datas[index].id;
            router.push({path:'material_info',query:{uuid:uuid}});
        }
    },
    mounted: function () {
        var _this=this;
